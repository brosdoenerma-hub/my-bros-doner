const products=[
 ...
 {id:'pommes',name:'Pommes',category:'snack',price:4,tag:'Snack',desc:'Goldbraun, knusprig und frisch gesalzen. Ketchup oder Mayo kostenlos.',image:'pommes.jpg',config:'fries'},
...
function openConfig(id){
 ...
 if(current.config==='fries')html+=`<div class="option-group"><h3>Kostenlose Soße wählen</h3><div class="chips">${chip('radio','sauce-none','Ohne Soße',true)}${chip('radio','sauce-ketchup','Ketchup')}${chip('radio','sauce-mayo','Mayonnaise')}</div></div><div class="option-group"><h3>Weitere Soßen <small>(je +1,00 €)</small></h3><div class="chips">${sauces.map((x,i)=>chip('checkbox',`sauce-extra-${i}`,`${x} · +${euro(1)}`,false,1)).join('')}</div></div>`;
 else if(current.config==='coffee')html+=`<div class="option-group"><h3>Kostenlose Wünsche</h3><div class="chips">${chip('checkbox','Milch','Milch')}${chip('checkbox','Zucker','Zucker')}</div></div>`;
...
function updateConfigPrice(){let base=itemBasePrice(current);const size=document.querySelector('#configForm input[name=size]:checked');if(size)base=Number(size.dataset.extra)+(current.deposit||0);document.querySelectorAll('#configForm input[type=checkbox]:checked').forEach(i=>base+=Number(i.dataset.extra||0));document.querySelector('#configPrice').textContent=euro(base)}
function addConfigured(){
 const checked=[...document.querySelectorAll('#configForm input:checked')]; let price=itemBasePrice(current); const size=checked.find(i=>i.name==='size');if(size)price=Number(size.dataset.extra)+(current.deposit||0);checked.filter(i=>i.type==='checkbox').forEach(i=>price+=Number(i.dataset.extra||0));
 let details=checked.filter(i=>!i.value.startsWith('size')).map(i=>i.value.startsWith('sauce')||i.value.startsWith('drink')?i.nextElementSibling.textContent:i.value).join(', ');
