...
function openCart(){
 document.querySelector('#cartItems').innerHTML=cart.length?cart.map((x,i)=>`<div class="cart-item"><div><strong>${x.name}</strong><small>${[x.size,x.details].filter(Boolean).join(' · ')}</small></div><strong>${euro(x.price)}</strong><button class="remove" data-remove="${i}" aria-label="Entfernen">×</button></div>`).join(''):'<div class="empty">Dein Warenkorb ist noch leer.</div>';
