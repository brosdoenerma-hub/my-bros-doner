...
function sauceTierPrice(checked){const count=checked.filter(i=>i.dataset.sauceTier==='basic').length;return current.sauceMode==='oneFree'?Math.max(0,count-1)*.5:current.sauceMode==='paidBasic'?count*.5:0}
function updateConfigPrice(){let base=itemBasePrice(current);const size=document.querySelector('#configForm input[name=size]:checked'),checked=[...document.querySelectorAll('#configForm input:checked')];if(size)base=Number(size.dataset.extra)+(current.deposit||0);checked.filter(i=>i.type==='checkbox').forEach(i=>base+=Number(i.dataset.extra||0));base+=sauceTierPrice(checked);document.querySelector('#configPrice').textContent=euro(base)}
function addConfigured(){
 ...
 let details=checked.filter(i=>!i.value.startsWith('size')).map(i=>i.value.startsWith('sauce')||i.value.startsWith('drink')||i.dataset.sauceTier?i.nextElementSibling.textContent:i.value).join(', ');
...
function updateCart(){const total=cart.reduce((s,x)=>s+x.price,0);document.querySelector('#cartCount').textContent=cart.length;document.querySelector('#floatingCount').textContent=cart.length;document.querySelector('#floatingTotal').textContent=euro(total);document.querySelector('#floatingCart').hidden=!cart.length;const send=document.querySelector('#sendWhatsapp'),blocked=mode==='Lieferung'&&total<22;send.classList.toggle('minimum-blocked',blocked);send.title=blocked?'Mindestbestellwert für Lieferung: 22,00 €':''}
function openCart(){
